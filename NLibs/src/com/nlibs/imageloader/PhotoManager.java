...
//        };
                    ...
                    if (photoTask.getImageURL() == localURL)
                        ...
                        switch (inputMessage.what) {
                            ...
                            case DOWNLOAD_STARTED:
                                //localView.setStatusResource(photoTask.getPhotoView().mEmptyResID);
                                break;
        ...
        switch (state) {
            ...
            case TASK_COMPLETE:
                ...
                if (photoTask.isCacheEnabled()) {
                	...
                	NLibsCache.getInstance(mContext).putBitmapToCache(photoTask.getImageURL().toString(), photoTask.getImage(), TYPE.BOTH);
                }
     ...
     */
    public void removeDownload(PhotoTask downloaderTask, URL pictureURL) {
     ...
     */
    public PhotoTask startDownload(
            PhotoView imageView,
            boolean cacheFlag) {
        ...
        if (null == downloadTask) {
            downloadTask = new PhotoTask(mContext);
        }
        ...
        Bitmap b = NLibsCache.getInstance(mContext).getBitmapFromCache(downloadTask.getImageURL().toString(), TYPE.BOTH);
...
//        if (null == downloadTask.getByteBuffer()) {
        if (null == b) {
            ...
            imageView.setStatusResource(downloadTask.getPhotoView().mEmptyResID);
        ...
        // The image was cached, so no download is required.
        	...
        	/**
        	 * Vì đã có bitmap trong cache, ta chỉ cần hiển thị lên, không giống như cache mảng byte (phải đi decode lại)
        	 */
