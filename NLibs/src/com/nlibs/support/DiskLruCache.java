...
public class DiskLruCache {
	...
	private final int maxCacheItemSize = 200; // 64 item default
	...
	public static DiskLruCache openCache(File cacheDir,
			long maxByteSize) {
		if (!cacheDir.exists()) {
			cacheDir.mkdir();
		}
		...
		/* && cacheDir.getUsableSpace() > maxByteSize */) {
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED
				|| !CommonUtils.isExternalStorageRemovable()) {
			File f = CommonUtils.getExternalCacheDir(context);
			if (null != f) {
